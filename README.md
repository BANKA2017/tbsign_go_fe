...
首次访问时会跳转到 `/add-base-path` 添加首个 API basePath：
...
- 找回密码 `/reset-password` 要求后端能够通过电子邮件的设置检测，但页面开启不等于重置邮件可以正常发出或者能被收到
- 各个插件的设置页 (`/plugin/` 目录内的页面) 需要站点开放了对应插件才会启用
...
- [x] ~~双 Token (RefreshToken, AccessToken)~~ 放弃 token，回归使用 `cookie`
- [ ] 工具箱，端点都准备好了，就差前端没写
- [x] 亮色模式非常亮……
